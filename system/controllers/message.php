...
EOT;
    ...
    case 'send_bulk_ajax':
        ...
        $createPaymentLink = function ($customerId) {
            if (empty($customerId)) {
                return '';
            }

            $token = User::generateToken($customerId, 1);
            if (empty($token['token'])) {
                return '';
            }
            ...
            return APP_URL . '/?_route=home&recharge=' . $tur['id'] . '&uid=' . urlencode($token['token']);
        };
        ...
        foreach ($customers as $customer) {
            ...
            if ($customerId && (empty($customer['fullname']) || empty($customer['username']) || !isset($customer['phonenumber']))) {
                $customerData = ORM::for_table('tbl_customers')->find_one($customerId);
                if ($customerData) {
                    $customer = array_merge($customerData->as_array(), $customer);
            ...
            $paymentLink = '';
            if (strpos($message, '[[payment_link]]') !== false) {
                $paymentLink = $createPaymentLink($customerId);
            }

            $currentMessage = str_replace(
                ['[[name]]', '[[user_name]]', '[[phone]]', '[[company_name]]', '[[payment_link]]'],
                [
                    $customer['fullname'] ?? '',
                    $customer['username'] ?? '',
