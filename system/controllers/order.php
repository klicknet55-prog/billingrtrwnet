...
$getAutomaticDiscount = function ($amount, $gateway = null, $planId = null) {
    $amount = (float) $amount;
    if ($amount <= 0 || !function_exists('apply_discount')) {
        return array(
            'amount' => $amount,
            'discount' => 0,
        );
    ...
    $discountedAmount = (float) apply_discount($amount, $gateway, $planId);
...
switch ($action) {
    ...
    case 'pay':
        ...
        if ($plan['validity_unit'] == 'Period') {
            $disableInvoiceAttribute = isset($config['disable_invoice_attribute']) && $config['disable_invoice_attribute'] == 'yes';
            if (!$disableInvoiceAttribute) {
                $add_inv = User::getAttribute("Invoice", $id_customer);
                if (!empty($add_inv) && $add_inv != 0) {
                    $base_price = (float) $add_inv;
                }
            }
        }
        $discount_result = $getAutomaticDiscount($base_price, 'balance', $plan['id']);
        $subtotal_after_discount = $discount_result['amount'] + $add_cost;
        ...
        if ($tax_enable === 'yes') {
            $tax = Package::tax($subtotal_after_discount, $tax_rate);
        } else {
            $tax = 0;
        }
        ...
        if ($plan && $plan['enabled'] && $user['balance'] >= $total_cost) {
            ...
            if ($discount_result['discount'] > 0) {
                $invoiceNote .= Lang::T('Payment Discount') . ' : ' . Lang::moneyFormat($discount_result['discount']) . "\n";
            }

            if (Package::rechargeUser($user['id'], $router_name, $plan['id'], 'Customer', 'Balance', $invoiceNote)) {
    ...
    case 'gateway':
        ...
        $pluginDiscount = 0;
        $totalDue = (float) $plan['price'] + (float) $add_cost;
        $tax = 0;
        ...
        $pgs = array_values(array_filter(array_map('trim', explode(',', $config['payment_gateway']))));
        if (count($pgs) == 0) {
        ...
        if (count($pgs) > 0) {
            if (!$isCustomCheckout && $_c['enable_balance'] != 'no' && $plan['type'] != 'Balance') {
                $balanceDiscount = $getAutomaticDiscount((float) $plan['price'], 'balance', $plan['id']);
                $balanceSubtotal = $balanceDiscount['amount'] + (float) $add_cost;
                $balanceTax = 0;
                if ($tax_enable === 'yes') {
                    $balanceTax = Package::tax($balanceSubtotal, $tax_rate);
                }
                $balanceTotal = $balanceSubtotal + $balanceTax;

                if ($_user['balance'] >= $balanceTotal) {
                    $gatewayOptions[] = array(
                        'value' => 'balance',
                        'label' => Lang::T('Balance') . ' ' . Lang::moneyFormat($_user['balance']),
                        'discount' => $balanceDiscount['discount'],
                        'discount_formatted' => Lang::moneyFormat($balanceDiscount['discount']),
                        'tax' => $balanceTax,
                        'tax_formatted' => Lang::moneyFormat($balanceTax),
                        'total' => $balanceTotal,
                        'total_formatted' => Lang::moneyFormat($balanceTotal),
                    );
                }
            }

            foreach ($pgs as $pg) {
                $gatewaySubtotal = $totalDue;
                $gatewayDiscount = array(
                    'amount' => $gatewaySubtotal,
                    'discount' => 0,
                );
                $gatewayTax = 0;

                if (!$isCustomCheckout) {
                    $gatewayDiscount = $getAutomaticDiscount((float) $plan['price'], $pg, $plan['id']);
                    $gatewaySubtotal = $gatewayDiscount['amount'] + (float) $add_cost;
                    if ($tax_enable === 'yes') {
                        $gatewayTax = Package::tax($gatewaySubtotal, $tax_rate);
                    }
                    $gatewaySubtotal += $gatewayTax;
                }

                $gatewayOptions[] = array(
                    'value' => $pg,
                    'label' => ucwords($pg),
                    'discount' => $gatewayDiscount['discount'],
                    'discount_formatted' => Lang::moneyFormat($gatewayDiscount['discount']),
                    'tax' => $gatewayTax,
                    'tax_formatted' => Lang::moneyFormat($gatewayTax),
                    'total' => $gatewaySubtotal,
                    'total_formatted' => Lang::moneyFormat($gatewaySubtotal),
                );
            }

            if (!empty($gatewayOptions)) {
                $pluginDiscount = $gatewayOptions[0]['discount'];
                $tax = $gatewayOptions[0]['tax'];
                $totalDue = $gatewayOptions[0]['total'];
            }
            ...
            $ui->assign('plan', $plan);
            $ui->assign('pgs', $pgs);
            $ui->assign('gateway_options', $gatewayOptions);
            $ui->assign('plugin_discount', $pluginDiscount);
            $ui->assign('tax', $tax);
            $ui->assign('total_due', $totalDue);
    ...
    case 'buy':
        ...
        switch (_post('custom')) {
            ...
            default:
                ...
                $discounted_base_price = max($base_price - (float) $discount, 0);
                $discount_result = $getAutomaticDiscount($discounted_base_price, $gateway, $plan['id']);
                $subtotal_before_tax = $discount_result['amount'] + $add_cost;
                // Tax calculation start
                $tax_enable = isset($config['enable_tax']) ? $config['enable_tax'] : 'no';
                $tax_rate_setting = isset($config['tax_rate']) ? $config['tax_rate'] : null;
                $custom_tax_rate = isset($config['custom_tax_rate']) ? (float)$config['custom_tax_rate'] : null;
                ...
                if ($tax_enable === 'yes') {
                    $tax = Package::tax($subtotal_before_tax, $tax_rate);
                }
                // Tax calculation stop
                if (empty($id)) {
                    $d = ORM::for_table('tbl_payment_gateway')->create();
                    ...
                    $d->price = $subtotal_before_tax + $tax;
                    $d->discount_amount = $discount_result['discount'];
                    $d->created_date = date('Y-m-d H:i:s');
                    $d->status = 1;
                    $d->save();
                    $id = $d->id();
